...
const canWeDeliver = (zipCode) => {
  if (deliveryAreaZipCodes.includes(zipCode)) {
    return "You are qualified for delivery!";
  } else {
    return "sorry, not within distance.";
  }
};
...
const canWeDeliverTwo = (zipCode) => {
  for (i = 0; i < deliveryAreaZipCodes.length; i++) {
    if (zipCode === deliveryAreaZipCodes[i]) {
      return "You are qualified for delivery!";
    } else return "Sorry, you're not within distance for delivery.";
  }
};
...
// const newDeals = (obj) => {
//   return obj.replace("15", "10");
// };

// console.log(newDeals(deals));
